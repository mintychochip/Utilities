 ...
 * <ul>
 *   <li>Typed, priority-ordered listeners</li>
 *   <li>Cancellable events via {@link Cancellable} / {@link AbstractCancellableEvent}</li>
 *   <li>Synchronous posting ({@link #post}) and asynchronous posting ({@link #postAsync})</li>
 *   <li>Functional registration and annotation-driven registration ({@link Subscribe})</li>
 *   <li>Per-listener {@code ignoreCancelled} filtering</li>
 *   <li>Inheritance dispatch: a listener for a supertype receives subtypes</li>
 * </ul>
 */
...
public interface EventBus {
     ...
     * @param ignoreCancelled if true, this listener is skipped when a {@link Cancellable} event is already cancelled
     ...
     */
    @NotNull List<Subscription> register(@NotNull Object instance);

    /**
     * Unregisters all {@link Subscribe} methods previously registered from {@code instance}.
     * No-op if the instance was never registered.
     */
     ...
     *
     * <p>Listeners are invoked in priority order (LOWEST → MONITOR).  Cancellable
     * filtering is respected per-listener.  Exceptions from one listener do not
     * prevent subsequent listeners.</p>
     *
    ...
    /**
     * Posts {@code event} asynchronously using the bus default executor.
     * Listeners are still invoked sequentially in priority order so cancellation
     * propagates deterministically, but the whole chain runs off the caller thread.
     *
