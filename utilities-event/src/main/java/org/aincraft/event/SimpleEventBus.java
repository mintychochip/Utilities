 ...
 *
 * <p>Thread-safe: subscriptions and posting may be interleaved concurrently.  Posting
 * collects a snapshot of matching listeners so registration changes during dispatch do
 * not affect the in-flight event.</p>
 */
final class SimpleEventBus implements EventBus {
    ...
    private final ConcurrentHashMap<Class<? extends Event>, CopyOnWriteArrayList<ListenerEntry<?>>> byType =
            new ConcurrentHashMap<>();
    private final ConcurrentHashMap<Object, List<Subscription>> instanceBindings = new ConcurrentHashMap<>();
    ...
    SimpleEventBus(@NotNull Executor defaultAsyncExecutor) {
        this.defaultAsyncExecutor = Preconditions.checkNotNull(defaultAsyncExecutor, "defaultAsyncExecutor");
    }
    ...
    public <E extends Event> @NotNull Subscription subscribe(
        ...
        ListenerEntry<E> entry = new ListenerEntry<>(eventType, priority, ignoreCancelled, executor, listener, seq);
    ...
    public @NotNull List<Subscription> register(@NotNull Object instance) {
        ...
        for (Method method : methods) {
            ...
            @SuppressWarnings("unchecked")
            ListenerEntry<Event> entry = new ListenerEntry<>(
        ...
        if (!created.isEmpty()) {
            instanceBindings.merge(instance, created, (oldList, newList) -> {
    ...
    @Override
    @SuppressWarnings("unchecked")
    public <E extends Event> @NotNull CompletableFuture<E> postAsync(@NotNull E event, @Nullable Executor executor) {
        Preconditions.checkNotNull(event, "event");
    ...
    private void handleListenerException(Throwable t, ListenerEntry<?> entry, Event event) {
        ...
        System.err.println("[EventBus] Listener " + entry.listener + " for " + entry.eventType.getSimpleName()
                + " threw: " + cause);
    ...
    private List<ListenerEntry<?>> collectMatching(Class<? extends Event> eventClass) {
        ...
        for (Map.Entry<Class<? extends Event>, CopyOnWriteArrayList<ListenerEntry<?>>> e : byType.entrySet()) {
            if (e.getKey().isAssignableFrom(eventClass)) {
                out.addAll(e.getValue());
            }
        }
        out.sort(Comparator
                .comparingInt((ListenerEntry<?> le) -> le.priority.ordinal())
    ...
    private static final class ListenerEntry<E extends Event> {
        ...
        ListenerEntry(Class<E> eventType, EventPriority priority, boolean ignoreCancelled,
                      Executor executor, EventListener<? super E> listener, long sequence) {
    ...
    private final class SubscriptionImpl implements Subscription {
        ...
        @Override public @NotNull Class<? extends Event> eventType() { return entry.eventType; }
        @Override public @NotNull EventPriority priority() { return entry.priority; }
        @Override public boolean ignoreCancelled() { return entry.ignoreCancelled; }
